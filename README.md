...
Personal site. SvelteKit, prerendered to static files, served from S3 behind
CloudFront.
...
[`infra/edge/rewrite.js`](infra/edge/rewrite.js) and is imported directly by the
local server and the tests, so the rule is verified rather than duplicated.
...
| `docs/` | [Publishing](docs/publishing.md), [margin notes](docs/margin-notes.md) |
...
Pushing a post does not publish it, because the site has to rebuild to pick it
up. See [docs/publishing.md](docs/publishing.md) for the trigger, and the
fallbacks if it is not wired up.
